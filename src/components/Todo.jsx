...
const[cameraFlip, setCameraFlip] = useState("user");
...
return (
  <>
    {!imgSrc && (
      <Webcam audio={false} ref={webcamRef} screenshotFormat="image/jpeg" photoConstraints={photoConstraints}/>
    )}
    ...
    <div className="btn-group">
      ...
      {!imgSrc && (
        <button
          type="button"
          className="btn"
          onClick={() => setCameraFlip(prev => prev === "user" ? "environment" : "user")}>
          Flip Camera 
        </button> 
      )}
      {imgSrc && (
